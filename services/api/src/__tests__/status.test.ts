...
describe('POST /api/v1/searches/status', () => {
  ...
  it('does not leak days_over_typical_phase on 404 (no phase-slip hints to anonymous callers)', async () => {
    // days_over_typical_phase is the magnitude of how far the current phase has
    // run past its typical-max benchmark — the exact-days companion to
    // current_phase_on_pace. Like the pacing signals it derives from, the
    // integer is keyed by the current phase, so observing it on the 404 path
    // would let an anonymous caller infer both that the search exists AND that
    // its current phase has slipped past its expected window. Success shape only.
    const res = await fetch(`${baseUrl}/api/v1/searches/status`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    assert.equal(res.status, 404);
    const body = await res.json();
    assert.ok(!('days_over_typical_phase' in body), 'days_over_typical_phase must not leak on 404');
    assert.ok(!('data' in body), 'must not include data on 404');
  });
  ...
  it('does not leak placement_window_meets_target on 404 (negative paths return no data)', async () => {
    // The verdict joins the client's own target start date to how far the
    // search still has to run, so observing it on the unauthenticated path
    // would let an anonymous caller infer both that the search exists AND
    // whether it is expected to land in time for the seat to be filled.
    const res = await fetch(`${baseUrl}/api/v1/searches/status`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    assert.equal(res.status, 404);
    const body = await res.json();
    assert.ok(!('placement_window_meets_target' in body), 'placement_window_meets_target must not leak on 404');
    assert.ok(!('data' in body), 'must not include data on 404');
  });
  ...
  it('does not leak target_start_margin_weeks on 404 (negative paths return no data)', async () => {
    // The weeks rounding of that same buffer discloses exactly what the day
    // count does, just coarser: observing it on the unauthenticated path would
    // let an anonymous caller infer both that the search exists AND roughly how
    // much slack it has against the client's own deadline.
    const res = await fetch(`${baseUrl}/api/v1/searches/status`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    assert.equal(res.status, 404);
    const body = await res.json();
    assert.ok(!('target_start_margin_weeks' in body), 'target_start_margin_weeks must not leak on 404');
    assert.ok(!('data' in body), 'must not include data on 404');
  });
