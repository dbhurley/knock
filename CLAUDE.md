...
POST   /api/v1/searches/status            # Public client-facing status lookup (search_number + contact_email, no API key) — returns phase + canonical phase_explainer, progress %, next milestone, last activity, activity_count_last_7d, is_stalled pacing flag, phase_duration_typical (min/max days for current phase). Cache-Control: no-store, private on every response shape including 404.
