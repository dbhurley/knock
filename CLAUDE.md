...
- **Canonical `is_on_track` health verdict (v1.48 stickiness)** — `POST /api/v1/searches/status` now also returns `is_on_track`, a single server-computed boolean that answers "is the search healthy right now?": the AND of `current_phase_on_pace === true` (the current phase hasn't slipped past its typical-max benchmark) and `!is_stalled` (it hasn't gone quiet for a fortnight). The status page already combined those two signals client-side to decide whether to render the gold "on pace" tag on the current-phase pacing line; surfacing the combination server-side makes that read one source of truth — same rationale as `all_phases_on_pace` (v1.37) collapsing the `phases_on_pace`/`phases_benchmarked` comparison into one boolean. The planned status-change reminder email / PDF (roadmap #4) can now open a reassuring "your search is on track" line off one flag instead of re-ANDing the two signals itself. `null` in exactly the same states as `current_phase_on_pace` — terminal/non-progressing/`placed` phases, or any phase without a typical duration — so a paused, closed, or benchmark-less search never reads a misleading verdict; positive-only by design like `current_phase_on_pace`, so a phase that has slipped past its benchmark or gone quiet reads `false`, not a bare absence. The status page now prefers this field for the gold-tag gate (falling back to the local `current_phase_on_pace === true && is_stalled !== true` conjunction only on older API versions) — byte-identical behavior, one fewer client-side derivation. Covered by a new negative-path 404 test: the boolean is keyed to the current phase, so observing it on the unauthenticated path would let an anonymous caller infer both that the search exists AND whether it's progressing on schedule.
- **`appendPaceTag()` helper for the status page's gold "on pace" tag (v1.48 hygiene)** — the current-phase pacing line and the "just wrapped" glance each built the identical gold `pace-tag` span inline (a leading space text node for screen-reader legibility + a `<span class="pace-tag">on pace</span>`). They now both call one `appendPaceTag(el)` helper defined once at the top of `renderStatus()`, so the tag's markup and its a11y-space contract live in a single place instead of being duplicated per site. The journey-archive's own on-pace tag (a distinct `jl-pace` class with different structure) is intentionally left as-is. Byte-identical output — pure frontend readability refactor, matching the codebase's established one-source-of-truth hygiene (`daysToWeeks()`, `DAY_MS`).
- **Negative-path 404 coverage for `is_on_track` (v1.48 hardening)** — a new negative-path test asserts the `is_on_track` health verdict is absent on the unauthenticated 404 path, locking the no-enumeration contract for the field in CI alongside the `current_phase_on_pace` / `phase_percent` pacing signals it's derived from. Observing it would let an anonymous caller infer both that the search exists AND whether it's progressing on schedule.

### Recently Shipped (2026-07-14)
- **Canonical `last_activity_type` on the status response (v1.47 stickiness)** — `POST /api/v1/searches/status` now also returns `last_activity_type`, the machine-readable `activity_type` enum of the most recent public-visible activity, surfaced top-level alongside `last_activity_at` and `last_activity_summary` (all three hoisted from the same `latest` row, so no extra query). The summary is free-text human copy ("Candidate presented to committee"); the *type* is the canonical enum a consumer branches on. It directly pre-paves roadmap #4's per-type reminder emails: the listener that turns a new activity into an outbound email ("a candidate was just presented" vs "a committee interview was scheduled" vs "your search advanced a phase") can key off this one canonical field instead of reaching into `recent_activities[0].activity_type`. Same one-source-of-truth rationale as `last_activity_at` / `last_activity_summary`; `null` when there's no public activity yet, mirroring them. Pure API pre-pave (the headline card already derives its glyph from `recent_activities[0]`), same pre-pave-only precedent as v1.46's `activity_delta_7d` and v1.30's `phases_remaining`. Covered by a new negative-path 404 test: the enum is keyed to a real search, so observing it on the unauthenticated path would let an anonymous caller infer both that the search exists AND what kind of update it last saw.
- **Velocity-trend chip quotes the canonical `activity_delta_7d` magnitude (v1.47 stickiness)** — `activity_delta_7d` (v1.46) shipped as a pure API pre-pave with no page consumer; the status page now consumes it. The velocity-trend chip's hover/screen-reader description previously quoted only the prior-week absolute ("2 updates the week before — pipeline is picking up"); it now appends the week-over-week *magnitude* off the canonical signed delta — "…(3 more than the week before)" / "…(2 fewer than the week before)" — the exact "3 more than the week before" phrasing roadmap #4's reminder email wants, now shown one source of truth off the same integer the email will quote instead of the page re-subtracting the two window counts. Null-safe: on an older API version missing the field the suffix is simply omitted and the description is unchanged. Extends the v1.26 velocity-chip accessibility pass — the magnitude rides in the same `title` + `aria-label` a committee member on a screen reader already hears. Pure frontend, no API change.
- **Negative-path 404 coverage for `last_activity_type` + the raw `status` field (v1.47 hardening)** — a new negative-path test asserts `last_activity_type` (the new recency-type enum) and the raw `status` code are both absent on the unauthenticated 404 path. `status` is the single most phase-revealing field on the success shape — even more direct than the derived `phase_label` / `phase_step` already covered — yet it had no dedicated 404-leak assertion, relying only on the generic no-`data` check. Locks the no-enumeration contract for both in CI the same way the v1.35 identity-field, v1.43 raw-date, and v1.44 `days_in_phase` tests already do: observing either would let an anonymous caller infer that the search exists (and, for `status`, exactly which phase it's in / for `last_activity_type` what kind of update it last saw).

### Recently Shipped (2026-07-13)
- **Canonical `activity_delta_7d` week-over-week change (v1.46 stickiness)** — `POST /api/v1/searches/status` now also returns `activity_delta_7d`, the signed week-over-week change in public-visible activity (`activity_count_last_7d − activity_count_prev_7d`). It's the raw magnitude the whole velocity family is derived from: `velocity_trend` (v1.17) categorizes this delta against a ±2 dead-band, and `is_ramping_up` (v1.40) keys off the prior-window count — but the *magnitude itself* was computed server-side (for the trend classification) and then discarded. Surfacing it completes the family so the planned status-change reminder email / PDF (roadmap #4) can open with "5 updates this week — 3 more than the week before" off one canonical integer instead of re-subtracting the two window counts it already carries. Same one-source-of-truth rationale as `velocity_trend` / `is_ramping_up` themselves, and maximally surgical — it reuses the `delta` already computed in the handler for the trend classification, no extra query or work. Always a number (never null), like the two window counts it's derived from. Pure API pre-pave (no page consumer needed — the velocity chip already reads "up from N" off the prior-window count), same pre-pave-only precedent as v1.34's `typical_*` benchmark fields and v1.30's `phases_remaining`. Covered by a new negative-path 404 test: the integer is keyed to a real search's tempo, so observing it on the unauthenticated path would let an anonymous caller infer both that the search exists AND how its tempo is changing week over week.
- **One `VELOCITY_TREND_DEADBAND` constant for the ±2 trend dead-band (v1.46 hygiene)** — the week-over-week `velocity_trend` classification (v1.17) open-coded its dead-band as the bare literal `2` at both gate sites (`delta >= 2` → accelerating, `delta <= -2` → cooling). They now both read one module-level `VELOCITY_TREND_DEADBAND = 2` constant, so the "a trend only fires when the count moved by at least 2" rule lives in a single place instead of being duplicated per branch — a future tweak can't leave the accelerating threshold at a different magnitude than the cooling one. Pure readability/one-source-of-truth refactor — byte-identical output — matching the codebase's established constant-hoist hygiene (`DAY_MS`, `PHASE_TOTAL`, `WEEKS_THRESHOLD_DAYS`, `TARGET_TERMINAL_STATUSES`). Covered by the existing status-endpoint tests.
- **Negative-path 404 coverage for `activity_delta_7d` (v1.46 hardening)** — the new signed week-over-week delta is the raw magnitude of a search's tempo change, so a new negative-path test asserts it is absent on the unauthenticated 404 path, locking the no-enumeration contract for the field in CI alongside the rest of the velocity family (`activity_count_last_7d`, `activity_count_prev_7d`, `velocity_trend`, `is_ramping_up`). Observing it would let an anonymous caller infer both that the search exists AND how its tempo is moving week over week.

### Recently Shipped (2026-07-12)
- **Canonical `placement_followup_percent` within-window completion (v1.45 stickiness)** — `POST /api/v1/searches/status` now also returns `placement_followup_percent`, the within-window completion percent as a 0–100 integer: `placement_age_days` measured against the 90-day `PLACEMENT_FOLLOWUP_DAYS` window, clamped to `[0, 100]`. It's the post-placement analogue of `phase_percent` (v1.39, which does the same for the current in-flight phase): the placement card stays live for a full 90 days, but neither the day counters (`placement_followup_days_remaining` counting down, `placement_age_days` counting up) nor the age tag answers "how far *through* the follow-up window am I?" — the one honest proportional glance for the window. Surfacing it makes the placement card carry a number that walks forward on every return visit across the whole window (a fresh reason to revisit even after a search closes), and lets the planned post-placement reminder email / PDF (roadmap #4) quote "16% through the follow-up window" off the same integer the card renders. The status page folds it into the follow-up countdown line — "…76 days remaining · 16% through the window" — appended only while the window is still open (omitted once complete, and simply not appended on older API versions). `null` unless placed, exactly like the other placement fields. Covered by the existing placement-window negative-path 404 test (extended to assert the new field): the integer is keyed to the placed terminal, so observing it on the unauthenticated path would let an anonymous caller infer both that the search exists AND that it has reached placement.
- **`DAY_MS` constant on the status page (v1.45 hygiene)** — `public/status.html` did day-granularity date math against the bare `86,400,000` millisecond literal at four sites (`fmtRelative()`'s day bucket, and the placement-age / engagement-age / target-start countdown *fallback* paths that fire only on older API versions). They now all read one module-level `DAY_MS = 86400000` constant, so the ms→days unit lives in a single place on the frontend instead of being open-coded per site — the frontend mirror of the API's own `DAY_MS` hoist (v1.42) and the same one-source-of-truth rationale as the v1.42 frontend `daysToWeeks()` helper. Byte-identical to the prior literals; the minute/hour literals in `fmtRelative()` are deliberately left as their own units. Pure frontend refactor, no API change.
- **Negative-path 404 coverage for `placement_followup_percent` (v1.45 hardening)** — the new within-window percent is keyed to the placed terminal, so the existing placement-window 404-leak test was extended to assert it is absent on the unauthenticated 404 path, locking the no-enumeration contract for the field in CI alongside `placed_at` / `placement_followup_until` / `placement_age_days` / `placement_age_weeks` and the other placement signals. Observing it would let an anonymous caller infer both that the search exists AND that it has reached placement.

### Recently Shipped (2026-07-11)
- **One `WEEKS_THRESHOLD_DAYS` constant for the fortnight display boundary (v1.44 hygiene)** — every days→weeks field on the status response reads in weeks only once its span is at or past a fortnight and in exact days below that (`weeks_since_last_activity`, `estimated_weeks_remaining`, `weeks_until_next_milestone`, `engagement_age_weeks`, `placement_followup_weeks_remaining`, `placement_age_weeks`), but that 14-day boundary was open-coded as the bare literal `14` at six separate gate sites in the status handler. They now all read one module-level `WEEKS_THRESHOLD_DAYS = 14` constant, so the fortnight rule lives in a single place instead of being duplicated per field — a future tweak can't leave one field switching to weeks at a different threshold than the others. Pure readability/one-source-of-truth refactor — byte-identical output — matching the codebase's established constant-hoist hygiene (`DAY_MS`, `PHASE_TOTAL`, `TARGET_TERMINAL_STATUSES`). The target-start countdown's deliberately-wider `> 30` ("more than a month out") boundary is intentionally left as its own literal, not folded into this constant. Covered by the existing status-endpoint tests.
- **Journey-summary denominator prefers the canonical `phase_total` (v1.44 hygiene)** — the status page's collapsed "The full search journey · 3 of 8 phases complete" summary hardcoded its denominator as the frontend's own `FORWARD_PHASES.length`, even though every other phase-count read on the page (the pinned-tab step count, the progress bar, the ARIA progressbar label) already prefers the API's canonical `phase_total`. The summary now reads `d.phase_total` too (falling back to `FORWARD_PHASES.length` only on older API versions), so a future phase added to the journey can't leave this one line reading "3 of 8" while the rest of the page reads a different total. Byte-identical today (both are 8). Pure frontend, no API change — same one-source-of-truth rationale as the v1.42 `daysToWeeks()` frontend-helper pass.
- **Negative-path 404 coverage for `days_in_phase` (v1.44 hardening)** — `days_in_phase` is the anchorless count of days the search has sat in its current phase — the raw pacing signal the "In this phase for N days" line and the `current_phase_on_pace` / `phase_percent` fields all derive from — but it was a success-shape field with no dedicated 404-leak assertion, relying only on the generic no-`data` check. A new negative-path test now asserts it is absent on the unauthenticated 404 path, locking the no-enumeration contract for the in-phase-duration figure in CI the same way the v1.42 `progress_percent` and v1.43 raw-date tests already do. Observing it would let an anonymous caller infer both that the search exists AND how long it has been sitting in its current phase.

### Recently Shipped (2026-07-10)
- **`computeProgressPercent()` uses the `isProgressingPhase()` helper (v1.43 hygiene)** — the v1.34 `isProgressingPhase()` extraction centralised the "in a forward phase but not yet `placed`" predicate that gates every forward-looking field (`days_in_phase`, `is_stalled`, `current_phase_on_pace`, the next-milestone ETA), replacing four open-coded copies of `PUBLIC_STATUS_FORWARD.includes(status) && status !== 'placed'` — but `computeProgressPercent()` still open-coded that exact expression in its intra-phase-smoothing branch, missed by the original pass. It now calls `isProgressingPhase(status)` too, so a future phase-model change can't leave the progress-bar math using a subtly different notion of "still progressing" than the fields around it. Pure refactor — byte-identical output (the predicate is identical), covered by the existing status-endpoint tests. Same one-source-of-truth rationale as the original v1.34 extraction.
- **Decorative headline activity glyph is `aria-hidden` (v1.43 a11y)** — the "Latest update" card's activity-type glyph (`→ + ★ # ☎`, set as real DOM text via `iconEl.textContent`) was announced by screen readers as part of the label — "star Latest update" instead of "Latest update". The span is now `aria-hidden="true"` so assistive tech skips the purely-decorative glyph while sighted clients still see it. (The timeline rows below already render their glyph via CSS `content: attr(data-icon)`, which screen readers skip — the headline was the one glyph rendered as real text, so it was the one that leaked into the accessibility tree.) Same a11y-as-stickiness rationale as the v1.20 progress-bar, v1.26 velocity-chip/breakdown-strip, and v1.38 on-pace-tag passes — the status page is the one Knock surface clients return to unprompted, so every committee member should read it cleanly, not most of them. Pure frontend, no API change.
- **Negative-path 404 coverage for the raw scheduling/timestamp fields (v1.43 hardening)** — `target_start_date` (the raw ISO date the client wants the role filled — the most directly-revealing scheduling field, more so than the `days_until_target_start` / `weeks_until_target_start` countdowns already tested), `status_changed_at` (the raw current-phase entry timestamp), and `opened_at` (the raw engagement-open timestamp) were success-shape fields with no dedicated 404-leak assertion, relying only on the generic no-`data` check. A new negative-path test now asserts all three are absent on the unauthenticated 404 path, locking the no-enumeration contract for the raw date fields in CI the same way the v1.35 identity-field and v1.42 `progress_percent` tests already do. Observing any of them would let an anonymous caller infer both that the search exists AND when the client wants the role filled / when the search last moved / how long it has been running.

### Recently Shipped (2026-07-09)
- **One `daysToWeeks()` helper for the status page's fallback rounding (v1.42 hygiene)** — `public/status.html` open-coded the same `Math.max(1, Math.round(days / 7))` days→weeks conversion at eight *fallback* call sites (the weeks reads for engagement age, placement age, follow-up remaining, last-activity recency, next-milestone countdown, target-start countdown, and the estimated-weeks range). Every one of those prefers the API's canonical weeks integer (`engagement_age_weeks`, `placement_age_weeks`, `weeks_since_last_activity`, `weeks_until_next_milestone`, `weeks_until_target_start`, `estimated_weeks_remaining`, `placement_followup_weeks_remaining`) and only falls back to local rounding on an older API version — but the fallback math was duplicated eight times, so a future tweak could leave the page rounding differently from the server. The page now routes them all through one `daysToWeeks()` helper that mirrors the API's own `daysToWeeks()` (`services/api/src/routes/searches.ts`) exactly, so the client fallback and the canonical server value can never disagree. Byte-identical to the prior inline math at every site (the two variant sites — the `Math.max(wLo, …)` estimated-range upper bound and the target-start branch that only fires past 30 days — both land on the same value because the floor-at-1 is a no-op there). Pure frontend refactor, same one-source-of-truth rationale as the server-side v1.32 extraction.
- **`DAY_MS` named constant in the status handler (v1.42 hygiene)** — the status handler did day-granularity date math against the bare `86_400_000` millisecond literal in ten places (`days_in_phase`, `days_since_last_activity`, `engagement_age_days`, each `phase_history` duration, `placement_age_days`, the completion-window and next-milestone ETA projections, the placement follow-up countdown, and the target-start countdown). They now all read one module-level `DAY_MS = 86_400_000` constant, so the ms→days unit lives in a single place instead of being open-coded at each site. Pure readability/one-source-of-truth refactor — byte-identical output — matching the codebase's established constant-hoist hygiene (`PHASE_TOTAL`, `TARGET_TERMINAL_STATUSES`). Covered by the existing status-endpoint tests.
- **Negative-path 404 coverage for `progress_percent` (v1.42 hardening)** — `progress_percent` is the 0–100 whole-journey completion figure the status page renders as the phase progress bar — a direct signal of how far a search has advanced — but it was a success-shape field with no dedicated 404-leak assertion, relying only on the generic no-`data` check. A new negative-path test now asserts it is absent on the unauthenticated 404 path, locking the no-enumeration contract for the whole-journey progress figure in CI the same way the phase-count and identity-field tests already do. Observing it would let an anonymous caller infer both that the search exists AND roughly how far along it is.

### Recently Shipped (2026-07-08)
- **Self-benchmarking "just wrapped" glance (v1.41 stickiness)** — the always-visible "Just wrapped: **Scoping** · 8 days · on pace" line under the current-phase pacing (from the canonical `latest_completed_phase`, v1.38) now quotes the phase's pace benchmark inline — "Just wrapped: **Scoping** · 8 of a typical 5–14 days · on pace" — using the `typical_min_days` / `typical_max_days` the API already carries on `latest_completed_phase` (v1.34) but the glance previously ignored. The single most-glanceable *backward* signal on the page was showing a bare elapsed count a return visitor couldn't anchor ("8 days" — fast? slow?); it now reads self-benchmarking, matching both the durational journey archive's own rows and the reminder-email phrasing CLAUDE.md already documents ("8 of a typical 5–14 days"). Prefers the canonical benchmark fields and falls back to the bare "N days" count on older API versions or a phase with no typical duration. Pure frontend, no API change.
- **Current-phase typical-duration lookup resolved once (v1.41 hygiene)** — the status handler open-coded the same `PUBLIC_STATUS_TYPICAL_DURATION[row.status]` map lookup in four places (the `current_phase_on_pace` flag, the `phase_percent` fraction, the next-milestone ETA, and the `phase_duration_typical` response field). They now all read one hoisted `currentPhaseTypical` const resolved right after `phase`, matching the handler's established "compute once" hygiene (the shared `nowMs` instant, the resolved search `id` reused across queries, the single activity `GROUP BY` scan, `PHASE_TOTAL`, `daysToWeeks()`). Pure refactor — byte-identical output (the const's `?? null` matches the response field's prior per-site `?? null`), covered by the existing status-endpoint tests.
- **Negative-path 404 coverage for `activity_count_prev_7d` (v1.41 hardening)** — the previous-window activity count is the other half of the week-over-week velocity signal, but it was the one field in that family (`activity_count_last_7d`, `velocity_trend`, `is_ramping_up` all already covered) lacking a dedicated 404-leak assertion — it relied only on the generic no-`data` check. A new negative-path test now asserts it is absent on the unauthenticated 404 path, locking the no-enumeration contract for the full velocity surface in CI the same way v1.40 did for `is_ramping_up`. Observing it would let an anonymous caller infer both that the search exists AND how its tempo was moving a week ago.
