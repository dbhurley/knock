...
- **Status (`/status`)**: Client search-status lookup. Verifies the visitor by reference number (KNK-YYYY-NNN, returned at intake) plus the email captured on the intake form, then renders a redacted progress view: position, school, current phase (1–8) with a one-line plain-English explainer, candidates identified, candidates presented to committee, currently interviewing, last-update date, days-in-phase pacing (with a `phase_duration_typical` benchmark like "typically 14–28 days"), an `is_stalled` pacing flag, a 7-day activity-velocity count, and a five-row activity timeline drawn from a redacted whitelist of `search_activities` types. The browser tab title is rewritten to `<ref> · <phase> | Knock` on load so pinned tabs stay legible. Auth-exempt at the API layer (`POST /api/v1/searches/status`); on any mismatch returns 404 without disclosing existence; the response carries `Cache-Control: no-store, private` on every response shape including 404 so personalized data never sits in shared caches and a leaked cache entry cannot disclose the verified/unverified status of a (ref, email) pair. The intake success screen deep-links here as `?ref=KNK-YYYY-NNN` so new clients land prefilled. Every server-side `status` change (via `PATCH /api/v1/searches/:id`) auto-writes a `status_change` row, so the timeline self-populates as the pipeline advances. This is the primary surface in the **stickiness strategy** — clients return to a Knock-controlled page throughout the 10–16-week engagement instead of waiting for an email.
...
POST   /api/v1/searches/status            # Public client status lookup (no API key) — body: { search_number, contact_email }; on email match, returns redacted phase + canonical phase_explainer, progress_percent, next_milestone_label, candidate pipeline counts, last_activity_at + last_activity_summary + last_activity_type + last_activity_label (whitelisted activity types only; the type is the machine-readable enum a consumer branches on and the label its resolved singular copy — "candidate presented" — so no surface has to turn the enum into English itself), activity_count_last_7d, is_stalled, days_in_phase, phase_duration_typical (min/max days benchmark); 404 otherwise. Cache-Control: no-store, private on every response shape including 404.
...
| 1.4 | 2026-05-01 | Knock Team | Status-page v1.3 (stickiness): `PATCH /api/v1/searches/:id` now auto-writes a `status_change` row to `search_activities` whenever the search's `status` field actually changes (`'Search advanced: <from> → <to>'`, `performed_by='system'`, `metadata.from`/`metadata.to`), so the public timeline self-populates as the pipeline advances — no longer dependent on Janet logging each transition. Status page now renders a one-line plain-English explainer per phase (kills the empty-state on early searches) and rewrites the browser tab title to `<ref> · <phase> | Knock` so pinned/duplicated tabs become at-a-glance reminders. The status response now sets `Cache-Control: no-store, private` to keep personalized search data out of shared caches; test suite extended with a header-property assertion |
| 1.5 | 2026-05-14 | Knock Team | Status-page v1.10 (stickiness): `POST /api/v1/searches/status` now returns `phase_duration_typical` (a `{ min_days, max_days }` benchmark for the current phase, e.g. `sourcing → 14–28`), so the pacing line on the status page reads "In this phase for 18 days (typically 14–28 days)" instead of being anchorless. `Cache-Control: no-store, private` is now set unconditionally at the start of the handler (including the 404 path) so a leaked cache entry cannot disclose the verified/unverified status of a `(ref, email)` pair. Status page suppresses the target-start countdown row entirely in terminal phases (`placed`/`cancelled`/`closed_no_fill`) — the "(N days past target)" line read punitively on a successful placement. Test coverage extended: `phase_duration_typical` must not leak on 404, and the Cache-Control assertion now applies to every response shape |
| 1.6 | 2026-07-25 | Knock Team | Status-page v1.54 (stickiness + hygiene + hardening): each completed `phase_history` entry (and `latest_completed_phase`) now carries `days_over_typical` — the exact-days magnitude a phase ran past its typical-max benchmark, the per-phase mirror of v1.52's `days_over_typical_phase` and the magnitude companion to that entry's `on_pace` boolean; positive-only and null when the phase landed on pace, so a flawless phase reads a bare absence. The status page's journey archive now renders completed phases as the dated range v1.53's `exited_at` made canonical ("Apr 22 → May 4 · 12 days · on pace") rather than an arrival date alone, matching the phrasing the planned reminder email quotes. The canonical status deep-link is resolved once per render and reused by the "Email Janet" body *and* the Google Calendar / .ics links, which previously rebuilt it from `location.origin` — so no surface on the page assembles the link itself. Test coverage hardened structurally: a new assertion requires the 404 body to carry exactly `error` + `message`, so any future field that leaks on the unauthenticated path fails CI whether or not it has a dedicated per-field test |
| 1.7 | 2026-07-27 | Knock Team | Status-page v1.56 (stickiness + hygiene + a11y): `POST /api/v1/searches/status` now returns `activity_labels`, an ordered `[{ type, singular, plural }]` catalog carrying the human wording *and* the narrative order for every client-visible activity type — making the three enum-keyed fields (`activity_breakdown`, `activity_breakdown_this_phase`, `last_activity_type`) self-describing, so the status page and the planned reminder email render "5 candidates sourced, 2 candidates presented" off one source of truth instead of each carrying its own copy of the wording. The status page resolves the catalog once per render and both breakdown surfaces read it, falling back to the built-in map on older API versions or a malformed payload. `PUBLIC_ACTIVITY_TYPES` (the redaction whitelist) is now derived from that catalog, so a new client-visible type is added in exactly one place. The three current-phase pace fields (`current_phase_on_pace`, `phase_percent`, `days_over_typical_phase`) now share one resolved `currentPhaseBenchmark` guard, making their documented "null in exactly the same states" contract structural rather than three separate copies of the same nested condition. Accessibility: the journey archive's dated range ("Apr 22 → May 4 · 12 days") now carries an `aria-label` spelling the arrow out — screen readers either skip the glyph or read it as "right arrow" |
| 1.8 | 2026-07-29 | Knock Team | Status-page v1.58 (stickiness + hygiene): the status page's headline "Latest update" card now takes its activity-type glyph from the canonical `last_activity_type` field (v1.47's pre-pave, previously unconsumed) rather than reaching into `recent_activities[0]` — the card's other two reads already came from the top-level `last_activity_*` fields, so all three now resolve the same update through one path, matching how roadmap #4's reminder-email listener is documented to branch. API hygiene: the next-phase preview family (`next_milestone_label`, `next_phase_explainer`, `next_phase_duration_typical`) is now resolved from one `phase_catalog` entry instead of three independent keyed map lookups, so the preview can't disagree with the journey overview the page renders from that same catalog; the index bound reads the hoisted `PHASE_TOTAL`. Frontend hygiene: the fortnight days→weeks display boundary, open-coded as a bare `14` at six gates, is now one `WEEKS_THRESHOLD_DAYS` constant mirroring the API's v1.44 hoist (the target-start countdown's wider "more than a month out" rule is deliberately left separate). All three changes are byte-identical in output — the next-phase equivalence was verified across all twelve statuses including the `offer → placed` edge |
| 1.9 | 2026-07-30 | Knock Team | Status-page v1.59 (stickiness + hygiene + hardening): the full-journey archive's *current* phase row now carries its live day count and, when the search is on track, the same positive tag the completed rows earn — "Started May 04 · 18 days so far · on pace" — so the durational archive is continuous through the present instead of going blank on the one row a return visitor is actually sitting in; both reads come from fields already on the response (`days_in_phase`, canonical `is_on_track`), positive-only and absent wherever `days_in_phase` is null (placed / paused / cancelled). `POST /api/v1/searches/status` now also returns `next_milestone_phase`, the raw phase code of the next phase — closing an asymmetry where every other phase named on the response carries both a code and human copy while the next phase was named only in English, so roadmap #4's reminder email can branch on which phase is coming off an enum instead of string-matching the display label; resolved off the same `phase_catalog` entry the label / explainer / typical duration already come from, null in the same states as `next_milestone_label`, and covered by a new negative-path 404 test. Hygiene: the `PUBLIC_STATUS_PHASES[x]?.label ?? x` conversion, open-coded at four sites (the phase catalog, both ends of `describeStatusChange()`, and each `phase_history` entry), is now one `phaseLabelFor()` helper; on the page, the on-track verdict is resolved once per render and shared by the pacing line and the new journey row, and the journey rows' done/current/upcoming marker reads each catalog entry's own canonical `step` rather than its position in the rendered list. All hygiene changes are byte-identical in output; the frontend change was verified across the on-pace, off-pace, day-zero, `placed`, and missing-`phase_catalog` cases |
| 1.10 | 2026-08-02 | Knock Team | Status-page v1.62 (stickiness + hygiene + hardening): `POST /api/v1/searches/status` now also returns `target_start_margin_days`, the signed day count of buffer between the late end of the estimated placement window and the client's target start date (`days_until_target_start − estimated_days_remaining.max_days`) — the exact-days magnitude companion to v1.61's `placement_window_meets_target` boolean, the same way `days_over_typical_phase` is the companion to `current_phase_on_pace` and `activity_delta_7d` to `velocity_trend`; signed rather than positive-only (both directions are meaningful and neither is redundant with another field), derived from the same two canonical integers so `margin >= 0` exactly when the verdict is true, and null in exactly the same states. The status page folds the magnitude into the gold placement-window tag — "~3 weeks ahead of your target start" instead of a bare verdict — in weeks past a fortnight and exact days below it, positive-only and falling back to the bare tag on older API versions. Hygiene: the client-visible activity-type whitelist is now bound to the two activity queries from a module-level `PUBLIC_ACTIVITY_TYPE_LIST` rather than an `Array.from()` rebuilt on every status request (matching the v1.35 `TERMINAL_STATUSES` hoist), and the journey summary's two remaining `var` declarations are `const` like their neighbours. Covered by a new negative-path 404 test alongside the v1.54 structural envelope assertion — the margin joins the client's own deadline to how far the search still has to run, so observing it on the unauthenticated path would let an anonymous caller infer both that the search exists AND how much slack it has |
| 1.11 | 2026-08-05 | Knock Team | Status-page v1.65 (stickiness + a11y) and deploy-pipeline correctness. `POST /api/v1/searches/status` now also returns `last_activity_label`, the resolved singular human copy for `last_activity_type` — closing the last client-visible enum handed to consumers without its copy beside it (`status`/`phase_label`, `search_urgency`/`search_urgency_label`, `next_milestone_phase`/`next_milestone_label` and every phase entry's `phase`/`label` were already paired; v1.56's `activity_labels` made this copy *derivable* but only by scanning the catalog for the matching entry). Resolved from that same catalog via a `PUBLIC_ACTIVITY_LABEL_BY_TYPE` map so the two can never disagree; null when there's no public activity yet and for an unrecognised type. Accessibility: the headline "Latest update" card's activity-type glyph is correctly `aria-hidden` (v1.43), which left the card's *kind* of update visible to sighted clients and invisible to screen readers — the label now carries a `title` + `aria-label` naming it ("Latest update: candidate presented"), read from the new canonical field and falling back to the local catalog on older API versions. **Deploy pipeline:** the workflow's health check curled port 3000 — OpenClaw/Janet, a separate systemd service — rather than the API on 4000, so the deploy gate verified a process the workflow never touches and an API that failed to come up would still have reported success; and its `docker compose exec -T app npm run db:migrate` step referenced a service and an npm script that both do not exist. The migrate step is removed rather than repaired: this repo has no migration-ledger table and the early migrations are not idempotent (`002_create_schools.sql` opens with a bare `CREATE TABLE schools`), so a blanket re-run would abort on every deploy — auto-migration needs a ledger first (roadmap #16). Both test harnesses' default `API_URL` corrected from 3000 to 4000 for the same wrong-port reason |
| 1.12 | 2026-08-06 | Knock Team | Status-page v1.66 (stickiness + a11y + hygiene): `POST /api/v1/searches/status` now also returns `velocity_trend_label`, the resolved human copy for the `velocity_trend` enum (`accelerating` → "Picking up", `steady` → "Steady", `cooling` → "Easing off", `quiet` → "Quiet"). v1.60 surfaced `search_urgency_label` on the stated principle that no client-visible enum should be handed over without its English beside it and claimed it was the last such case — `velocity_trend` (v1.17) was not covered: it is the field the status page's velocity chip renders, and the words naming the category lived only in that chip's branch ladder, so roadmap #4's reminder email would have had to pick its own. Resolved from a module-level `PUBLIC_VELOCITY_TREND_LABELS` catalog typed against a new exhaustive `VelocityTrend` union, so the lookup always resolves and the field is always a string (unlike `search_urgency_label`, whose raw enum comes from client intake and may be absent). Pure API pre-pave — the chip's contextual sentence ("2 updates the week before — pipeline is picking up") stays the page's own copy — covered by extending the existing velocity-family negative-path 404 test. Accessibility: the reference-number pill carried a *static* `aria-label` ("Reference number — click to copy"), and because `aria-label` replaces an element's own text, the one string on the page a client is meant to quote back to Janet or read aloud on a committee call was the one string assistive tech could not reach; it is now set per render carrying the resolved ref ("Reference number KNK-2026-001 — click to copy"). Hygiene: the Share-link button — the surface whose entire job is handing the status link to a board chair — was the last one rebuilding it from `location.origin`, after v1.54 collapsed the mailto / Google Calendar / .ics sites into one resolved `statusLink`; the resolved link is now stashed beside the ref/email the Refresh button already carries, so a shared link and a calendared reminder can't diverge where `PUBLIC_BASE_URL` differs from the serving origin |
